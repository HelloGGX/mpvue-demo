...
fly.interceptors.request.use((request) => {
  ...
  request.data = qs.stringify(request.body.data)
...
class Request {
   createError = (code, resp) => {
    ...
    handleRequest = (request) => {
     ...
     handleAll = (request) => { // promisory一个promise工厂
       let args = []
       for (let i = 0; i < request.length; i++) {
         args.push(`data${i}`)
       }
       return new Promise((resolve, reject) => {
         fly.all(request).then(fly.spread((...args) => {
           resolve(args)
         })).catch((err) => {
           console.log(err)
   ...
   * GET类型的网络请求
  **/
     get (url, data) { // data是对象
       return this.handleRequest(fly.get(url, {data: data}, {
         showLoading: true
       }))
     }
  /**
   * POST类型的网络请求
  **/
     post (url, data) { // data是对象
       return this.handleRequest(fly.post(url, {data: data}, {
         showLoading: true
       }))
     }
  /**
   * DELETE类型的网络请求
  **/
     delete (url, data) { // data是对象
       return this.handleRequest(fly.delete(url, {data: data}, {
         showLoading: true
       }))
     }
}
